...
SELECT 
    ...
    NVL(mvrv.ID_VUELO, rv.ID_VUELO) AS ID_VUELO,
    NVL(mvrv.CLASE_ASIENTO_RESERVA, rv.CLASE_ASIENTO_RESERVA) AS CLASE_ASIENTO_RESERVA,
    rv.FECHA_CREACION,
    rv.FECHA_MODIFICACION,
    NVL(mvrv.ESTADO_RESERVA, rv.ESTADO_RESERVA) AS ESTADO_RESERVA,
    rv.FECHA_FORMALIZACION,
    rv.ERRORES
FROM FORMACION_TICARUM.RESERVA_VUELO rv LEFT JOIN FORMACION_TICARUM.VWEXT_RESERVA_VUELO mvrv
ON rv.ID_RESERVA_FORMALIZADA = mvrv.ID
UNION
SELECT 
    vwrv.ID,
...
WHERE NOT EXISTS (  SELECT 1  FROM FORMACION_TICARUM.RESERVA_VUELO rv WHERE rv.ID_RESERVA_FORMALIZADA = vwrv.ID);
