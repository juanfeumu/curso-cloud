...
@Component
public class ListarVuelosQueryHandler implements QueryHandler<Optional<Page<VueloAmpliadoDTO>>, ListarVuelosQuery> {
	...
	public ListarVuelosQueryHandler( VuelosReadRepository vuelosRepository, ReservasVueloReadRepository reservasVueloRepository ) {
		this.vuelosReadRepository = vuelosRepository;
		this.reservasVueloReadRepository = reservasVueloRepository;
	}

	@Override
	public Optional<Page<VueloAmpliadoDTO>> handle( ListarVuelosQuery query ) throws Exception {
		...
		return Optional.of( vuelos.map( v -> ApplicationMapper.vueloToDTO( v, vuelosReserva.get( v.getId() ) ) ) );
	}
